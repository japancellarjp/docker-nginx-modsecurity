...
ENV NGX_MODSECURITY_VERSION v1.0.1
ENV NGX_NJS_VERSION 0.4.1
ENV NGX_GEOIP2_VERSION 3.3
ENV NGX_NDK_VERSION 0.3.1
...
# coreruleset
RUN cd /opt && \
    wget -q -O coreruleset-${MODSECURITY_CRS_VERSION}.tar.gz https://github.com/coreruleset/coreruleset/archive/v${MODSECURITY_CRS_VERSION}.tar.gz && \
    tar xzf coreruleset-${MODSECURITY_CRS_VERSION}.tar.gz && \
    rm -f coreruleset-${MODSECURITY_CRS_VERSION}.tar.gz && \
    ln -s coreruleset-${MODSECURITY_CRS_VERSION} coreruleset

# njs
RUN cd /opt && \
    hg clone -r ${NGX_NJS_VERSION} http://hg.nginx.org/njs && \
    cd /opt/njs && \
    hg up
...
# coreruleset
RUN mkdir -p /etc/coreruleset
COPY --from=build /opt/coreruleset/crs-setup.conf.example /opt/coreruleset/crs-setup.conf
COPY --from=build /opt/coreruleset/rules /opt/coreruleset/rules
RUN cd /opt/coreruleset/rules && \
